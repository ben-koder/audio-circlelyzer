    ...
    <div class="flex flex-col gap-6">
        ...
        @if (hasSettingsUI(entry.key)) {
          <div class="card bg-base-100 shadow-sm border border-base-300">
            <div class="card-body p-4">
              <h3 class="card-title text-lg">{{ getVisualizationName(entry.key) }} <span class="text-sm font-normal opacity-70">({{ entry.key }})</span></h3>
      ...
      @if (getEntries().length === 0) {
        <div class="alert alert-info">
          <span>No configurable visualizations found in this context.</span>
        </div>
      }
    </div>
  `,
...
export class VisualizationSettingsComponent implements AfterViewInit {
  ...
  getVisualizationTypeId(key: string): string | undefined {
    const ctx = this.context();
    if (!ctx?.visualizations?.get) return undefined;
    return ctx.visualizations.get(key);
  }

  hasSettingsUI(key: string): boolean {
    const typeId = this.getVisualizationTypeId(key);
    if (!typeId) return false;
    const type = this.calculationManager.visualizationTypes.get(typeId);
  ...
  getVisualizationName(key: string): string {
    const typeId = this.getVisualizationTypeId(key);
